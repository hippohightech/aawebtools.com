# Translation Brief — twitter-video-downloader → French (fr)
...
> **After completion**: Save to `frontend/fr/telechargeur-twitter/index.html`, then run verify.
...
| Target file | `frontend/fr/telechargeur-twitter/index.html` |
| Target canonical | `https://aawebtools.com/fr/telechargeur-twitter/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/telechargeur-twitter/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/telechargeur-twitter/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/telechargeur-twitter/index.html
git commit -m "Regenerate fr/twitter-video-downloader (passes validator)"
```
