# Translation Brief — tiktok-downloader → French (fr)
...
> **After completion**: Save to `frontend/fr/telechargeur-tiktok/index.html`, then run verify.
...
| Target file | `frontend/fr/telechargeur-tiktok/index.html` |
| Target canonical | `https://aawebtools.com/fr/telechargeur-tiktok/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/telechargeur-tiktok/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/telechargeur-tiktok/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/telechargeur-tiktok/index.html
git commit -m "Regenerate fr/tiktok-downloader (passes validator)"
```
