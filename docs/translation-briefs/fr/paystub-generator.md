# Translation Brief — paystub-generator → French (fr)
...
> **After completion**: Save to `frontend/fr/generateur-bulletin-de-paie/index.html`, then run verify.
...
| Target file | `frontend/fr/generateur-bulletin-de-paie/index.html` |
| Target canonical | `https://aawebtools.com/fr/generateur-bulletin-de-paie/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/generateur-bulletin-de-paie/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/generateur-bulletin-de-paie/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/paystub-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/generateur-bulletin-de-paie/index.html
git commit -m "Regenerate fr/paystub-generator (passes validator)"
```
