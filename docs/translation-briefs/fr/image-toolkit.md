# Translation Brief — image-toolkit → French (fr)
...
> **After completion**: Save to `frontend/fr/boite-a-outils-images/index.html`, then run verify.
...
| Target file | `frontend/fr/boite-a-outils-images/index.html` |
| Target canonical | `https://aawebtools.com/fr/boite-a-outils-images/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/boite-a-outils-images/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/boite-a-outils-images/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/boite-a-outils-images/index.html
git commit -m "Regenerate fr/image-toolkit (passes validator)"
```
