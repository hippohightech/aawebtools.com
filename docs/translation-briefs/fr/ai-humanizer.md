# Translation Brief — ai-humanizer → French (fr)
...
> **After completion**: Save to `frontend/fr/humanisateur-ia/index.html`, then run verify.
...
| Target file | `frontend/fr/humanisateur-ia/index.html` |
| Target canonical | `https://aawebtools.com/fr/humanisateur-ia/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/humanisateur-ia/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/humanisateur-ia/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/humanisateur-ia/index.html
git commit -m "Regenerate fr/ai-humanizer (passes validator)"
```
