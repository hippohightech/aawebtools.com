# Translation Brief — ai-detector → French (fr)
...
> **After completion**: Save to `frontend/fr/detecteur-ia/index.html`, then run verify.
...
| Target file | `frontend/fr/detecteur-ia/index.html` |
| Target canonical | `https://aawebtools.com/fr/detecteur-ia/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/fr/detecteur-ia/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/fr/detecteur-ia/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL fr/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/fr/detecteur-ia/index.html
git commit -m "Regenerate fr/ai-detector (passes validator)"
```
