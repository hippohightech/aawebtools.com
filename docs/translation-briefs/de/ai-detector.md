# Translation Brief — ai-detector → German (de)
...
> **After completion**: Save to `frontend/de/ki-detektor/index.html`, then run verify.
...
| Target file | `frontend/de/ki-detektor/index.html` |
| Target canonical | `https://aawebtools.com/de/ki-detektor/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/ki-detektor/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/ki-detektor/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/ki-detektor/index.html
git commit -m "Regenerate de/ai-detector (passes validator)"
```
