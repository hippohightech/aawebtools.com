# Translation Brief — twitter-video-downloader → German (de)
...
> **After completion**: Save to `frontend/de/twitter-video-herunterladen/index.html`, then run verify.
...
| Target file | `frontend/de/twitter-video-herunterladen/index.html` |
| Target canonical | `https://aawebtools.com/de/twitter-video-herunterladen/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/twitter-video-herunterladen/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/twitter-video-herunterladen/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/twitter-video-herunterladen/index.html
git commit -m "Regenerate de/twitter-video-downloader (passes validator)"
```
