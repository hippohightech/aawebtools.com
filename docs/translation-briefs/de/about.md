# Translation Brief — about → German (de)
...
> **After completion**: Save to `frontend/de/ueber-uns/index.html`, then run verify.
...
| Target file | `frontend/de/ueber-uns/index.html` |
| Target canonical | `https://aawebtools.com/de/ueber-uns/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/ueber-uns/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/ueber-uns/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/about$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/ueber-uns/index.html
git commit -m "Regenerate de/about (passes validator)"
```
