# Translation Brief — invoice-generator → German (de)
...
> **After completion**: Save to `frontend/de/rechnungsgenerator/index.html`, then run verify.
...
| Target file | `frontend/de/rechnungsgenerator/index.html` |
| Target canonical | `https://aawebtools.com/de/rechnungsgenerator/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/rechnungsgenerator/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/rechnungsgenerator/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/rechnungsgenerator/index.html
git commit -m "Regenerate de/invoice-generator (passes validator)"
```
