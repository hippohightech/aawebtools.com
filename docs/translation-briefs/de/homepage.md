# Translation Brief — homepage → German (de)
...
> **After completion**: Save to `frontend/de/index.html`, then run verify.
...
| Target file | `frontend/de/index.html` |
| Target canonical | `https://aawebtools.com/de/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/index.html
git commit -m "Regenerate de/homepage (passes validator)"
```
