# Translation Brief — image-toolkit → German (de)
...
> **After completion**: Save to `frontend/de/bild-werkzeuge/index.html`, then run verify.
...
| Target file | `frontend/de/bild-werkzeuge/index.html` |
| Target canonical | `https://aawebtools.com/de/bild-werkzeuge/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/de/bild-werkzeuge/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/de/bild-werkzeuge/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL de/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/de/bild-werkzeuge/index.html
git commit -m "Regenerate de/image-toolkit (passes validator)"
```
