# Translation Brief — image-toolkit → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/ferramentas-imagem/index.html`, then run verify.
...
| Target file | `frontend/pt/ferramentas-imagem/index.html` |
| Target canonical | `https://aawebtools.com/pt/ferramentas-imagem/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/ferramentas-imagem/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/ferramentas-imagem/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/ferramentas-imagem/index.html
git commit -m "Regenerate pt/image-toolkit (passes validator)"
```
