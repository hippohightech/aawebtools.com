# Translation Brief — homepage → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/index.html`, then run verify.
...
| Target file | `frontend/pt/index.html` |
| Target canonical | `https://aawebtools.com/pt/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/index.html
git commit -m "Regenerate pt/homepage (passes validator)"
```
