# Translation Brief — about → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/sobre/index.html`, then run verify.
...
| Target file | `frontend/pt/sobre/index.html` |
| Target canonical | `https://aawebtools.com/pt/sobre/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/sobre/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/sobre/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/about$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/sobre/index.html
git commit -m "Regenerate pt/about (passes validator)"
```
