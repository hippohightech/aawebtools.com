# Translation Brief — twitter-video-downloader → Portuguese (pt)

> **Status**: Currently quarantined (`<meta robots noindex>`) on the live site.
> **Goal**: Produce a high-quality Portuguese translation that passes the strict validator.
> **After completion**: Save to `frontend/pt/baixar-twitter/index.html`, then run verify.
...
| Target language | Portuguese (Português) — `pt` |
| Target file | `frontend/pt/baixar-twitter/index.html` |
| Target canonical | `https://aawebtools.com/pt/baixar-twitter/` |
...
1. **`<html lang="pt">`** — must be set on the root element.
2. **`<title>`** — must exist, ≤65 characters.
3. **`<meta name="description">`** — must exist, 100–170 characters.
4. **`<link rel="canonical" href="https://aawebtools.com/pt/baixar-twitter/">`** — must exactly match.
...
9. **`<meta property="og:locale" content="pt_BR">`** — must be set.
...
After pasting the translated HTML into `frontend/pt/baixar-twitter/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/baixar-twitter/index.html
git commit -m "Regenerate pt/twitter-video-downloader (passes validator)"
```
