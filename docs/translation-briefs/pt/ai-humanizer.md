# Translation Brief — ai-humanizer → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/humanizador-ia/index.html`, then run verify.
...
| Target file | `frontend/pt/humanizador-ia/index.html` |
| Target canonical | `https://aawebtools.com/pt/humanizador-ia/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/humanizador-ia/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/humanizador-ia/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/humanizador-ia/index.html
git commit -m "Regenerate pt/ai-humanizer (passes validator)"
```
