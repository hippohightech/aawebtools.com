# Translation Brief — paystub-generator → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/gerador-holerite/index.html`, then run verify.
...
| Target file | `frontend/pt/gerador-holerite/index.html` |
| Target canonical | `https://aawebtools.com/pt/gerador-holerite/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/gerador-holerite/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/gerador-holerite/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/paystub-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/gerador-holerite/index.html
git commit -m "Regenerate pt/paystub-generator (passes validator)"
```
