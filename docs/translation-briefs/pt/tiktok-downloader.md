# Translation Brief — tiktok-downloader → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/baixar-tiktok/index.html`, then run verify.
...
| Target file | `frontend/pt/baixar-tiktok/index.html` |
| Target canonical | `https://aawebtools.com/pt/baixar-tiktok/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/baixar-tiktok/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/baixar-tiktok/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/baixar-tiktok/index.html
git commit -m "Regenerate pt/tiktok-downloader (passes validator)"
```
