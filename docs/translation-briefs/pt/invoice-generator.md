# Translation Brief — invoice-generator → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/gerador-faturas/index.html`, then run verify.
...
| Target file | `frontend/pt/gerador-faturas/index.html` |
| Target canonical | `https://aawebtools.com/pt/gerador-faturas/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/gerador-faturas/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/gerador-faturas/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/gerador-faturas/index.html
git commit -m "Regenerate pt/invoice-generator (passes validator)"
```
