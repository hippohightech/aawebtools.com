# Translation Brief — ai-detector → Portuguese (pt)
...
> **After completion**: Save to `frontend/pt/detector-ia/index.html`, then run verify.
...
| Target file | `frontend/pt/detector-ia/index.html` |
| Target canonical | `https://aawebtools.com/pt/detector-ia/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/pt/detector-ia/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/pt/detector-ia/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL pt/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/pt/detector-ia/index.html
git commit -m "Regenerate pt/ai-detector (passes validator)"
```
