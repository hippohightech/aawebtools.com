# Translation Brief — homepage → Spanish (es)
...
> **After completion**: Save to `frontend/es/index.html`, then run verify.
...
| Target file | `frontend/es/index.html` |
| Target canonical | `https://aawebtools.com/es/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/index.html
git commit -m "Regenerate es/homepage (passes validator)"
```
