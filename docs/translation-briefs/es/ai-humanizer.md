# Translation Brief — ai-humanizer → Spanish (es)
...
> **After completion**: Save to `frontend/es/humanizador-ia/index.html`, then run verify.
...
| Target file | `frontend/es/humanizador-ia/index.html` |
| Target canonical | `https://aawebtools.com/es/humanizador-ia/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/humanizador-ia/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/humanizador-ia/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/humanizador-ia/index.html
git commit -m "Regenerate es/ai-humanizer (passes validator)"
```
