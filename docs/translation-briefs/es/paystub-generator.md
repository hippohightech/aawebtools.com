# Translation Brief — paystub-generator → Spanish (es)
...
> **After completion**: Save to `frontend/es/generador-recibo-nomina/index.html`, then run verify.
...
| Target file | `frontend/es/generador-recibo-nomina/index.html` |
| Target canonical | `https://aawebtools.com/es/generador-recibo-nomina/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/generador-recibo-nomina/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/generador-recibo-nomina/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/paystub-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/generador-recibo-nomina/index.html
git commit -m "Regenerate es/paystub-generator (passes validator)"
```
