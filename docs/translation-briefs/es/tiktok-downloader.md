# Translation Brief — tiktok-downloader → Spanish (es)
...
> **After completion**: Save to `frontend/es/descargador-tiktok/index.html`, then run verify.
...
| Target file | `frontend/es/descargador-tiktok/index.html` |
| Target canonical | `https://aawebtools.com/es/descargador-tiktok/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/descargador-tiktok/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/descargador-tiktok/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/descargador-tiktok/index.html
git commit -m "Regenerate es/tiktok-downloader (passes validator)"
```
