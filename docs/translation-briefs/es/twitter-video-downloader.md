# Translation Brief — twitter-video-downloader → Spanish (es)
...
> **After completion**: Save to `frontend/es/descargador-twitter/index.html`, then run verify.
...
| Target file | `frontend/es/descargador-twitter/index.html` |
| Target canonical | `https://aawebtools.com/es/descargador-twitter/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/descargador-twitter/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/descargador-twitter/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/descargador-twitter/index.html
git commit -m "Regenerate es/twitter-video-downloader (passes validator)"
```
