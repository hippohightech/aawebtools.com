# Translation Brief — image-toolkit → Spanish (es)
...
> **After completion**: Save to `frontend/es/herramientas-imagenes/index.html`, then run verify.
...
| Target file | `frontend/es/herramientas-imagenes/index.html` |
| Target canonical | `https://aawebtools.com/es/herramientas-imagenes/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/herramientas-imagenes/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/herramientas-imagenes/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/herramientas-imagenes/index.html
git commit -m "Regenerate es/image-toolkit (passes validator)"
```
