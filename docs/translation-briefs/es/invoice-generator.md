# Translation Brief — invoice-generator → Spanish (es)
...
> **After completion**: Save to `frontend/es/generador-facturas/index.html`, then run verify.
...
| Target file | `frontend/es/generador-facturas/index.html` |
| Target canonical | `https://aawebtools.com/es/generador-facturas/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/es/generador-facturas/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/es/generador-facturas/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL es/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/es/generador-facturas/index.html
git commit -m "Regenerate es/invoice-generator (passes validator)"
```
