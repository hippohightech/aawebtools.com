# Translation Brief — invoice-generator → Indonesian (id)
...
> **After completion**: Save to `frontend/id/pembuat-faktur/index.html`, then run verify.
...
| Target file | `frontend/id/pembuat-faktur/index.html` |
| Target canonical | `https://aawebtools.com/id/pembuat-faktur/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/pembuat-faktur/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/pembuat-faktur/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/pembuat-faktur/index.html
git commit -m "Regenerate id/invoice-generator (passes validator)"
```
