# Translation Brief — ai-detector → Indonesian (id)
...
> **After completion**: Save to `frontend/id/pendeteksi-ai/index.html`, then run verify.
...
| Target file | `frontend/id/pendeteksi-ai/index.html` |
| Target canonical | `https://aawebtools.com/id/pendeteksi-ai/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/pendeteksi-ai/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/pendeteksi-ai/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/pendeteksi-ai/index.html
git commit -m "Regenerate id/ai-detector (passes validator)"
```
