# Translation Brief — twitter-video-downloader → Indonesian (id)
...
> **After completion**: Save to `frontend/id/unduh-twitter/index.html`, then run verify.
...
| Target file | `frontend/id/unduh-twitter/index.html` |
| Target canonical | `https://aawebtools.com/id/unduh-twitter/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/unduh-twitter/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/unduh-twitter/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/unduh-twitter/index.html
git commit -m "Regenerate id/twitter-video-downloader (passes validator)"
```
