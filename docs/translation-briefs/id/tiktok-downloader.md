# Translation Brief — tiktok-downloader → Indonesian (id)
...
> **After completion**: Save to `frontend/id/unduh-tiktok/index.html`, then run verify.
...
| Target file | `frontend/id/unduh-tiktok/index.html` |
| Target canonical | `https://aawebtools.com/id/unduh-tiktok/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/unduh-tiktok/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/unduh-tiktok/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/unduh-tiktok/index.html
git commit -m "Regenerate id/tiktok-downloader (passes validator)"
```
