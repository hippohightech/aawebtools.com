# Translation Brief — homepage → Indonesian (id)
...
> **After completion**: Save to `frontend/id/index.html`, then run verify.
...
| Target file | `frontend/id/index.html` |
| Target canonical | `https://aawebtools.com/id/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/index.html
git commit -m "Regenerate id/homepage (passes validator)"
```
