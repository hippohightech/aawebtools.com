# Translation Brief — image-toolkit → Indonesian (id)
...
> **After completion**: Save to `frontend/id/peralatan-gambar/index.html`, then run verify.
...
| Target file | `frontend/id/peralatan-gambar/index.html` |
| Target canonical | `https://aawebtools.com/id/peralatan-gambar/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/peralatan-gambar/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/peralatan-gambar/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/peralatan-gambar/index.html
git commit -m "Regenerate id/image-toolkit (passes validator)"
```
