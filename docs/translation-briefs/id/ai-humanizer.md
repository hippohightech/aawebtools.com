# Translation Brief — ai-humanizer → Indonesian (id)
...
> **After completion**: Save to `frontend/id/humanizer-ai/index.html`, then run verify.
...
| Target file | `frontend/id/humanizer-ai/index.html` |
| Target canonical | `https://aawebtools.com/id/humanizer-ai/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/id/humanizer-ai/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/id/humanizer-ai/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/humanizer-ai/index.html
git commit -m "Regenerate id/ai-humanizer (passes validator)"
```
