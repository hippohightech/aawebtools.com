# Translation Brief — paystub-generator → Indonesian (id)

> **Status**: Currently quarantined (`<meta robots noindex>`) on the live site.
> **Goal**: Produce a high-quality Indonesian translation that passes the strict validator.
> **After completion**: Save to `frontend/id/pembuat-slip-gaji/index.html`, then run verify.
...
| Target language | Indonesian (Bahasa Indonesia) — `id` |
| Target file | `frontend/id/pembuat-slip-gaji/index.html` |
| Target canonical | `https://aawebtools.com/id/pembuat-slip-gaji/` |
...
1. **`<html lang="id">`** — must be set on the root element.
2. **`<title>`** — must exist, ≤65 characters.
3. **`<meta name="description">`** — must exist, 100–170 characters.
4. **`<link rel="canonical" href="https://aawebtools.com/id/pembuat-slip-gaji/">`** — must exactly match.
...
9. **`<meta property="og:locale" content="id_ID">`** — must be set.
...
After pasting the translated HTML into `frontend/id/pembuat-slip-gaji/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL id/paystub-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/id/pembuat-slip-gaji/index.html
git commit -m "Regenerate id/paystub-generator (passes validator)"
```
