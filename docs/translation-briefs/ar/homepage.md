# Translation Brief — homepage → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/index.html`, then run verify.
...
| Target file | `frontend/ar/index.html` |
| Target canonical | `https://aawebtools.com/ar/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/index.html
git commit -m "Regenerate ar/homepage (passes validator)"
```
