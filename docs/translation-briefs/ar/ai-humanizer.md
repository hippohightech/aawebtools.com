# Translation Brief — ai-humanizer → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/ai-humanizer/index.html`, then run verify.
...
| Target file | `frontend/ar/ai-humanizer/index.html` |
| Target canonical | `https://aawebtools.com/ar/ai-humanizer/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/ai-humanizer/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/ai-humanizer/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/ai-humanizer/index.html
git commit -m "Regenerate ar/ai-humanizer (passes validator)"
```
