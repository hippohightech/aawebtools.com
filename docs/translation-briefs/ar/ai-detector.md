# Translation Brief — ai-detector → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/ai-detector/index.html`, then run verify.
...
| Target file | `frontend/ar/ai-detector/index.html` |
| Target canonical | `https://aawebtools.com/ar/ai-detector/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/ai-detector/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/ai-detector/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/ai-detector/index.html
git commit -m "Regenerate ar/ai-detector (passes validator)"
```
