# Translation Brief — twitter-video-downloader → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/twitter-video-downloader/index.html`, then run verify.
...
| Target file | `frontend/ar/twitter-video-downloader/index.html` |
| Target canonical | `https://aawebtools.com/ar/twitter-video-downloader/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/twitter-video-downloader/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/twitter-video-downloader/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/twitter-video-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/twitter-video-downloader/index.html
git commit -m "Regenerate ar/twitter-video-downloader (passes validator)"
```
