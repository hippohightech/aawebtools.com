# Translation Brief — invoice-generator → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/invoice-generator/index.html`, then run verify.
...
| Target file | `frontend/ar/invoice-generator/index.html` |
| Target canonical | `https://aawebtools.com/ar/invoice-generator/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/invoice-generator/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/invoice-generator/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/invoice-generator/index.html
git commit -m "Regenerate ar/invoice-generator (passes validator)"
```
