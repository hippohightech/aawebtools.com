# Translation Brief — tiktok-downloader → Arabic (ar)
...
> **After completion**: Save to `frontend/ar/tiktok-downloader/index.html`, then run verify.
...
| Target file | `frontend/ar/tiktok-downloader/index.html` |
| Target canonical | `https://aawebtools.com/ar/tiktok-downloader/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/ar/tiktok-downloader/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/ar/tiktok-downloader/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL ar/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/ar/tiktok-downloader/index.html
git commit -m "Regenerate ar/tiktok-downloader (passes validator)"
```
