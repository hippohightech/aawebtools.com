# Translation Brief — paystub-generator → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/paystub-generator/index.html`, then run verify.
...
| Target file | `frontend/hi/paystub-generator/index.html` |
| Target canonical | `https://aawebtools.com/hi/paystub-generator/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/paystub-generator/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/paystub-generator/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/paystub-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/paystub-generator/index.html
git commit -m "Regenerate hi/paystub-generator (passes validator)"
```
