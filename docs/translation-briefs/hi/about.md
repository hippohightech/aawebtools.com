# Translation Brief — about → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/about/index.html`, then run verify.
...
| Target file | `frontend/hi/about/index.html` |
| Target canonical | `https://aawebtools.com/hi/about/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/about/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/about/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/about$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/about/index.html
git commit -m "Regenerate hi/about (passes validator)"
```
