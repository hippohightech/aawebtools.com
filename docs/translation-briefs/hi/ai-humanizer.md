# Translation Brief — ai-humanizer → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/ai-humanizer/index.html`, then run verify.
...
| Target file | `frontend/hi/ai-humanizer/index.html` |
| Target canonical | `https://aawebtools.com/hi/ai-humanizer/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/ai-humanizer/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/ai-humanizer/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/ai-humanizer$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/ai-humanizer/index.html
git commit -m "Regenerate hi/ai-humanizer (passes validator)"
```
