# Translation Brief — invoice-generator → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/invoice-generator/index.html`, then run verify.
...
| Target file | `frontend/hi/invoice-generator/index.html` |
| Target canonical | `https://aawebtools.com/hi/invoice-generator/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/invoice-generator/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/invoice-generator/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/invoice-generator$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/invoice-generator/index.html
git commit -m "Regenerate hi/invoice-generator (passes validator)"
```
