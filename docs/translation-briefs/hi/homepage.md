# Translation Brief — homepage → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/index.html`, then run verify.
...
| Target file | `frontend/hi/index.html` |
| Target canonical | `https://aawebtools.com/hi/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/homepage$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/index.html
git commit -m "Regenerate hi/homepage (passes validator)"
```
