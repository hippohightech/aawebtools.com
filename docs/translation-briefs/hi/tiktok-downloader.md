# Translation Brief — tiktok-downloader → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/tiktok-downloader/index.html`, then run verify.
...
| Target file | `frontend/hi/tiktok-downloader/index.html` |
| Target canonical | `https://aawebtools.com/hi/tiktok-downloader/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/tiktok-downloader/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/tiktok-downloader/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/tiktok-downloader$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/tiktok-downloader/index.html
git commit -m "Regenerate hi/tiktok-downloader (passes validator)"
```
