# Translation Brief — ai-detector → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/ai-detector/index.html`, then run verify.
...
| Target file | `frontend/hi/ai-detector/index.html` |
| Target canonical | `https://aawebtools.com/hi/ai-detector/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/ai-detector/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/ai-detector/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/ai-detector$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/ai-detector/index.html
git commit -m "Regenerate hi/ai-detector (passes validator)"
```
