# Translation Brief — image-toolkit → Hindi (hi)
...
> **After completion**: Save to `frontend/hi/image-toolkit/index.html`, then run verify.
...
| Target file | `frontend/hi/image-toolkit/index.html` |
| Target canonical | `https://aawebtools.com/hi/image-toolkit/` |
...
4. **`<link rel="canonical" href="https://aawebtools.com/hi/image-toolkit/">`** — must exactly match.
...
After pasting the translated HTML into `frontend/hi/image-toolkit/index.html`, run:

```bash
node tools/translate/build.js 2>&1 | grep -A 6 "FAIL hi/image-toolkit$" || echo "PASS — page validates"
```
...
```bash
git add frontend/hi/image-toolkit/index.html
git commit -m "Regenerate hi/image-toolkit (passes validator)"
```
